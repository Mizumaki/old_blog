...
// TODO: anyの多用、本来はstates内でやるべきことが分離出来ていない。

interface IWindow { AMP: any; location: any; }
declare var window: IWindow;

type IProps = RouteComponentProps<{ main: string; sub: string; fileName: string; }>
...
class AMPDocument extends React.Component<IProps, IStates> {
  private ampReadyPromise_: any;
  private container_: any;
  private xhr_: any;
  private shadowAmp_: any;
  private shadowRoot_: any;
  private boundClickListener_: any;
  ...
  public fetchAndAttachAmpDoc_(url: string) {
    console.log('in fetch and attach amp doc')
    this.fetchDocument_(url).then(doc => {
      return this.ampReadyPromise_.then((amp: any) => {
  ...
  public hideUnwantedElementsOnDocument_(doc: any) {
  ...
  public fetchDocument_(url: string) {
